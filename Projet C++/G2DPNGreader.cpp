...
int  CORECreateTextureFromRGBA(byte * data, int width, int height);  
// convert RGB data to RGBA data
void ExtendRGBtoRGBA(std::vector<byte>& data);	
// Vertical-symetrystd::
void VsymetryRGBAImage(std::vector<byte>& data, int Width, int Height);
// any pixel with given RGB color value becomes transparent
void SetRGBATransparentPixels(std::vector<byte>& data, byte RA, byte GA, byte BA);
// call lodepng
