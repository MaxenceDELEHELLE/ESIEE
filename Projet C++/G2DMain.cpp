...
// d�tecte un changement de taille de la fen�tre et remet la taille initiale
...
double deltaTimeMS(std::chrono::system_clock::time_point now, 
...
void G2D::Run(void logic(GameData & G), void Render(const GameData & G), GameData & G,int nbCalltoLogicPerSec,bool HideCursor)
{
	...
	// glEnable(GL_POLYGON_SMOOTH); � �viter rendu erron�
	...
	myGameData       = & G;
	glutDisplayFunc(OPENGLRender);      // fonction appel�e lors d'un repaint
